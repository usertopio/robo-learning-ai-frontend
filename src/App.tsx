...
function AppContent() {
  ...
  const [lossHistory, setLossHistory] = useState<
    { epoch: number; loss: number; val_loss: number; map50: number }[]
  >([]);
  const [detResults, setDetResults] = useState<any[]>([]);
  const [sessionFrames, setSessionFrames] = useState(0);
  const [imageInference, setImageInference] = useState({
    running: false,
    current: 0,
    total: 0,
    filename: "",
  ...
  useEffect(() => {
    ...
    const handleTraining = (data: any) => {
      ...
      window.dispatchEvent(
        new CustomEvent("training-progress-update", { detail: updated }),
      );
    };
    ...
    const handleDetResults = (data: any) => {
      ...
      setDetResults(rows);
      window.dispatchEvent(
        new CustomEvent("det-results-update", { detail: rows }),
      );
    };
    const handleImageStart = (data: any) => {
      ...
      window.dispatchEvent(
        new CustomEvent("image-inference-start", { detail: data }),
      );
    };
    const handleImageProgress = (data: any) => {
      setImageInference((prev) => ({
        ...prev,
      ...
      window.dispatchEvent(
        new CustomEvent("image-inference-progress", { detail: data }),
      );
    };
    const handleImageDone = (data: any) => {
      setImageInference({
        running: false,
        current: data.total_images,
        total: data.total_images,
        filename: "Done!",
      });
      window.dispatchEvent(
        new CustomEvent("image-inference-done", { detail: data }),
      );
    };
    ...
    const handleDatasetStatus = (data: any) => {
      window.dispatchEvent(
        new CustomEvent("dataset-status-update", { detail: data }),
      );
    ...
    const handleArtifacts = (data: any) => {
      ...
      window.dispatchEvent(
        new CustomEvent("training-artifacts-update", { detail: data }),
      );
    };
  ...
  useEffect(() => {
    window.dispatchEvent(
      new CustomEvent("datasets-updated", { detail: datasets }),
    );
  }, [datasets]);
  ...
  return (
    <div
      className={`font-sans bg-[#F9FAFB] dark:bg-slate-950 text-slate-800 dark:text-slate-200 h-screen w-screen overflow-hidden flex flex-col ${isDark ? "dark" : ""}`}
    >
          ...
          <div className="h-9 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between px-5 text-xs text-slate-500 dark:text-slate-400 font-medium shrink-0 transition-colors z-10 shadow-[0_-2px_10px_rgba(0,0,0,0.02)]">
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1.5">
                <code className="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-[10px] font-bold text-slate-600 dark:text-slate-300">
                  {nodes.length}
                </code>{" "}
                Blocks
              </span>
            ...
            <div className="flex items-center gap-2">
              <span
                className={`w-2 h-2 rounded-full ${isPipelineComplete ? "bg-emerald-500" : "bg-amber-500"}`}
              ></span>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[9px] text-slate-400 italic">
                    Separate with commas. Press Enter to apply.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 text-center pt-2">
                  <div className="bg-white dark:bg-slate-800/80 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-sm">
                    <p className="text-[11px] text-indigo-500 dark:text-indigo-400 font-bold uppercase tracking-wider mb-1">
                      Epochs
                    </p>
                    ...
                    {trainingProgress.status === "training" && (
                      <p className="text-[10px] text-indigo-400 animate-pulse mt-1">
                        Training...
                      </p>
                    )}
                  ...
                  <div className="bg-white dark:bg-slate-800/80 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-sm">
                    <p className="text-[11px] text-orange-500 dark:text-orange-400 font-bold uppercase tracking-wider mb-1">
                      Loss (Train)
                    </p>
                    <p className="text-2xl font-black text-slate-800 dark:text-slate-100">
                    ...
                    <p className="text-[10px] text-slate-400 mt-1">
                      Val:{" "}
                      {trainingProgress.val_loss > 0
                        ? trainingProgress.val_loss.toFixed(4)
                        : "0.00"}
                    </p>
                  </div>
                  <div className="bg-white dark:bg-slate-800/80 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-sm">
                    <p className="text-[11px] text-purple-500 dark:text-purple-400 font-bold uppercase tracking-wider mb-1">
                      Model
                    </p>
                    <p className="text-lg font-black text-slate-800 dark:text-slate-100 truncate">
                      {modelVariant.split(" ")[0]}
                    </p>
                    <p className="text-[9px] text-slate-400 mt-1">
                      {modelVariant.includes("Nano")
                        ? "Ultra Fast"
                        : modelVariant.includes("Small")
                          ? "Balanced"
                          : modelVariant.includes("Medium")
                            ? "Accurate"
                            : "Professional"}
                    </p>
                  </div>
                  <div className="bg-white dark:bg-slate-800/80 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-sm">
                    <p className="text-[11px] text-cyan-500 dark:text-cyan-400 font-bold uppercase tracking-wider mb-1">
                      Frames
                    </p>
                    <p className="text-2xl font-black text-slate-800 dark:text-slate-100">
                      {sessionFrames}
                ...
                {frameHistory.length > 0 && (
                  <div className="space-y-2 pt-4">
                    ...
                    <div className="grid grid-cols-5 gap-2">
                      {frameHistory.length > 0 &&
                        frameHistory.slice(-5).map((frame, idx) => (
                          <div
                            key={idx}
                            className="aspect-square rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow"
                          >
                            <img
                              src={frame}
                              alt={`Test ${idx}`}
      ...
      <footer className="h-16 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 shrink-0 transition-colors shadow-[0_-5px_15px_-10px_rgba(0,0,0,0.05)] z-20 relative">
        <div className="text-xs text-slate-500 font-medium">
          {isPipelineComplete
            ? "Ready to train or run your model."
            : "Please connect Input ➔ AI Model ➔ Monitor to start inference."}
        </div>
        <div className="flex gap-3">
          <button
            onClick={handleTrain}
            className="flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-bold text-sm rounded-xl transition-all shadow-lg shadow-indigo-500/25 active:scale-[0.98]"
          >
          ...
          <button
            ...
            title={
              !isPipelineComplete ? "Please connect blocks properly first" : ""
            }
          ...
          >
            <span className="text-lg">{isAiSystemRunning ? "⏹️" : "▶️"}</span>{" "}
            {isAiSystemRunning ? "Stop AI" : "Start"}
          </button>
        </div>
      </footer>
    </div>
